...
client.on("message", async (message) => {
  ...
  message.channel.send({
    ...
    if (args[0] === "ekle") {
        ...
        \`${hedeftag}\` Tagı yasaklı taglar listesine eklendi.
        `))
    }
    if (args[0] === "liste") {
        let data = await db.get(`yasaklitaglar.${message.guild.id}`) || []
...
`))
    ...
    if (args[0] === "kaldır") {
        ...
        message.channel.send(embed.setDescription(`
    \`${hedeftag}\` Tagı yasaklı tag listesinden başarıyla kaldırıldı.
`))
    }
    if (!args[0]) {
        message.channel.send(embed.setDescription(`
${message.author}, Hatalı kullanım Örnek\n \`.yasaklıtag ekle/kaldır/liste\`
`))
    }
